...
> 💡 **macOS 首次打开提示**：由于当前 DMG 未做 Apple 代码签名，双击 Mynx 可能提示「已损坏，无法打开」。这是 macOS Gatekeeper 的隔离机制，应用本身没有问题。请按以下步骤操作（全程图形界面，无需打开终端输命令）：
>
> 1. 打开下载的 DMG，将 Mynx.app 拖到「应用程序」文件夹
> 2. 回到 DMG 窗口，找到 **`修复损坏.command`**，**右键 → 打开**（注意必须右键，普通双击仍会被拦）
> 3. 弹出的安全对话框点「打开」，终端会自动完成修复并弹出成功提示
> 4. 关掉终端窗口，现在双击「应用程序」里的 Mynx 即可正常使用
>
> 此修复只需执行一次。若第 2 步右键打开仍失败，可在终端执行 `xattr -cr /Applications/Mynx.app` 作为兜底。
